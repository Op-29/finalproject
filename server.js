...
// ใช้ path.join เพื่อให้โค้ดทำงานได้ทั้งบน Windows และ Linux
const upload = multer({ dest: path.join(__dirname, 'uploads/') }); // เก็บไฟล์ชั่วคราว
...
// แก้ไขการพิมพ์ผิด: 'module.exprots' เป็น 'module.exports'
module.exports = con;
...
app.post('/upload', upload.single('csvFile'), (req, res) => {
    // ใช้ `try...catch` เพื่อจับ error ที่อาจเกิดขึ้นจากการรัน spawn
    try {
        if (!req.file) {
            return res.status(400).json({ status: 'error', message: 'กรุณาเลือกไฟล์ที่จะอัปโหลด' });
        }

        const filePath = req.file.path;
        const python = spawn('python', ['ml_predict.py', filePath]);
        
        // ใช้ตัวแปรนี้เพื่อควบคุมไม่ให้ส่ง response ซ้ำซ้อน
        let sentResponse = false;
        let output = '';
        let errorOutput = '';

        // เพิ่มการจัดการ error event สำหรับ spawn โดยเฉพาะ
        // 'error' event จะถูก emit เมื่อไม่สามารถรัน process ได้ เช่น หา python.exe ไม่เจอ
        python.on('error', (err) => {
            if (sentResponse) return;
            sentResponse = true;
            console.error('Failed to start Python process:', err);
            res.status(500).json({ status: 'error', message: 'ไม่สามารถรันสคริปต์ Python ได้ โปรดตรวจสอบว่าได้ติดตั้ง Python และตั้งค่า PATH อย่างถูกต้อง' });
        });
        ...
        python.on('close', (code) => {
            ...
            // ถ้าส่ง response ไปแล้ว ไม่ต้องทำอะไร
            if (sentResponse) return;

            // ถ้า Python รันไม่สำเร็จ (exit code ไม่ใช่ 0)
            if (code !== 0) {
                console.error(`Python process exited with code ${code}. Error output: ${errorOutput}`);
                sentResponse = true;
                return res.status(500).json({ status: 'error', message: `การประมวลผล Python ล้มเหลว: ${errorOutput}` });
            }

            try {
                // ตรวจสอบ output ก่อนการ parse
                if (!output || output.trim() === "") {
                    console.warn("⚠️ Python returned empty output");
                    sentResponse = true;
                    return res.json({ status: 'no_data', message: 'ไม่พบข้อมูลในไฟล์ที่อัปโหลด' });
                }

                let results = JSON.parse(output);

                // กรองเอาแค่จังหวัดเชียงราย
                results = results.filter(row => row['Pro.จังหวัด']?.trim() === 'จ.เชียงราย');
                ...
                function combine_district_from_string(row) {
                    const map = {
                        'อ.เมืองเชียงราย': 1, 'อ.เวียงชัย': 2, 'อ.เชียงของ': 3, 'อ.เทิง': 4, 'อ.พาน': 5,
                        'อ.ป่าแดด': 6, 'อ.แม่จัน': 7, 'อ.เชียงแสน': 8, 'อ.แม่สาย': 9, 'อ.แม่สรวย': 10,
                        'อ.เวียงป่าเป้า': 11, 'อ.พญาเม็งราย': 12, 'อ.เวียงแก่น': 13, 'อ.ขุนตาล': 14,
                        'อ.แม่ฟ้าหลวง': 15, 'อ.แม่ลาว': 16, 'อ.เวียงเชียงรุ้ง': 17, 'อ.ดอยหลวง': 18,
                    };
                    const name = row['Pro.อำเภอ / เขต']?.trim(); // ป้องกันค่า null
                    return map[name] || null;
                }

                function normalize(str) {
                    return str?.trim()?.replace(/\s+/g, ''); // ใช้ Optional Chaining
                }

                function combine_subdistrict(row) {
                    const map = {
                        "อ.เมืองเชียงราย|ต.เวียง": 101, "อ.เมืองเชียงราย|ต.รอบเวียง": 102, "อ.เมืองเชียงราย|ต.บ้านดู่": 103,
                        "อ.เมืองเชียงราย|ต.นางแล": 104, "อ.เมืองเชียงราย|ต.แม่ข้าวต้ม": 105, "อ.เมืองเชียงราย|ต.แม่ยาว": 106,
                        "อ.เมืองเชียงราย|ต.สันทราย": 107, "อ.เมืองเชียงราย|ต.แม่กรณ์": 108, "อ.เมืองเชียงราย|ต.ห้วยชมภู": 109,
                        "อ.เมืองเชียงราย|ต.ห้วยสัก": 110, "อ.เมืองเชียงราย|ต.ริมกก": 111, "อ.เมืองเชียงราย|ต.ดอยลาน": 112,
                        "อ.เมืองเชียงราย|ต.ป่าอ้อดอนชัย": 113, "อ.เมืองเชียงราย|ต.ท่าสาย": 114, "อ.เมืองเชียงราย|ต.ดอยฮาง": 115,
                        "อ.เมืองเชียงราย|ต.ท่าสุด": 116, "อ.เวียงชัย|ต.เวียงชัย": 201, "อ.เวียงชัย|ต.ผางาม": 202,
                        "อ.เวียงชัย|ต.เวียงเหนือ": 203, "อ.เวียงชัย|ต.ดอนศิลา": 204, "อ.เวียงชัย|ต.เมืองชุม": 205,
                        "อ.เชียงของ|ต.เวียง": 301, "อ.เชียงของ|ต.สถาน": 302, "อ.เชียงของ|ต.ครึ่ง": 303,
                        "อ.เชียงของ|ต.บุญเรือง": 304, "อ.เชียงของ|ต.ห้วยซ้อ": 305, "อ.เชียงของ|ต.ศรีดอนชัย": 306,
                        "อ.เชียงของ|ต.ริมโขง": 307, "อ.เทิง|ต.เวียง": 401, "อ.เทิง|ต.งิ้ว": 402, "อ.เทิง|ต.ปล้อง": 403,
                        "อ.เทิง|ต.แม่ลอย": 404, "อ.เทิง|ต.เชียงเคี่ยน": 405, "อ.เทิง|ต.ตับเต่า": 406, "อ.เทิง|ต.หงาว": 407,
                        "อ.เทิง|ต.สันทรายงาม": 408, "อ.เทิง|ต.ศรีดอนไชย": 409, "อ.เทิง|ต.หนองแรด": 410,
                        "อ.พาน|ต.สันมะเค็ด": 501, "อ.พาน|ต.แม่อ้อ": 502, "อ.พาน|ต.ธารทอง": 503, "อ.พาน|ต.สันติสุข": 504,
                        "อ.พาน|ต.ดอยงาม": 505, "อ.พาน|ต.หัวง้ม": 506, "อ.พาน|ต.เจริญเมือง": 507, "อ.พาน|ต.ป่าหุ่ง": 508,
                        "อ.พาน|ต.ม่วงคำ": 509, "อ.พาน|ต.ทรายขาว": 510, "อ.พาน|ต.สันกลาง": 511, "อ.พาน|ต.แม่เย็น": 512,
                        "อ.พาน|ต.เมืองพาน": 513, "อ.พาน|ต.ทานตะวัน": 514, "อ.พาน|ต.เวียงห้าว": 515, "อ.ป่าแดด|ต.ป่าแดด": 601,
                        "อ.ป่าแดด|ต.ป่าแงะ": 602, "อ.ป่าแดด|ต.สันมะค่า": 603, "อ.ป่าแดด|ต.โรงช้าง": 604, "อ.ป่าแดด|ต.ศรีโพธิ์เงิน": 605,
                        "อ.แม่จัน|ต.แม่จัน": 701, "อ.แม่จัน|ต.จันจว้า": 702, "อ.แม่จัน|ต.แม่คำ": 703, "อ.แม่จัน|ต.ป่าซาง": 704,
                        "อ.แม่จัน|ต.สันทราย": 705, "อ.แม่จัน|ต.ท่าข้าวเปลือก": 706, "อ.แม่จัน|ต.ป่าตึง": 707, "อ.แม่จัน|ต.แม่ไร่": 708,
                        "อ.แม่จัน|ต.ศรีค้ำ": 709, "อ.แม่จัน|ต.จันจว้าใต้": 710, "อ.แม่จัน|ต.จอมสวรรค์": 711, "อ.เชียงแสน|ต.เวียง": 801,
                        "อ.เชียงแสน|ต.ป่าสัก": 802, "อ.เชียงแสน|ต.บ้านแซว": 803, "อ.เชียงแสน|ต.ศรีดอนมูล": 804,
                        "อ.เชียงแสน|ต.แม่เงิน": 805, "อ.เชียงแสน|ต.โยนก": 806, "อ.แม่สาย|ต.แม่สาย": 901, "อ.แม่สาย|ต.ห้วยไคร้": 902,
                        "อ.แม่สาย|ต.เกาะช้าง": 903, "อ.แม่สาย|ต.โป่งผา": 904, "อ.แม่สาย|ต.ศรีเมืองชุม": 905,
                        "อ.แม่สาย|ต.เวียงพางคำ": 906, "อ.แม่สาย|ต.บ้านด้าย": 907, "อ.แม่สาย|ต.โป่งงาม": 908,
                        "อ.แม่สรวย|ต.แม่สรวย": 1001, "อ.แม่สรวย|ต.ป่าแดด": 1002, "อ.แม่สรวย|ต.แม่พริก": 1003,
                        "อ.แม่สรวย|ต.ศรีถ้อย": 1004, "อ.แม่สรวย|ต.ท่าก๊อ": 1005, "อ.แม่สรวย|ต.วาวี": 1006,
                        "อ.แม่สรวย|ต.เจดีย์หลวง": 1007, "อ.เวียงป่าเป้า|ต.สันสลี": 1101, "อ.เวียงป่าเป้า|ต.เวียง": 1102,
                        "อ.เวียงป่าเป้า|ต.บ้านโป่ง": 1103, "อ.เวียงป่าเป้า|ต.ป่างิ้ว": 1104, "อ.เวียงป่าเป้า|ต.เวียงกาหลง": 1105,
                        "อ.เวียงป่าเป้า|ต.แม่เจดีย์": 1106, "อ.เวียงป่าเป้า|ต.แม่เจดีย์ใหม่": 1107, "อ.พญาเม็งราย|ต.แม่เปา": 1201,
                        "อ.พญาเม็งราย|ต.แม่ต๋ำ": 1202, "อ.พญาเม็งราย|ต.ไม้ยา": 1203, "อ.พญาเม็งราย|ต.เม็งราย": 1204,
                        "อ.พญาเม็งราย|ต.ตาดควัน": 1205, "อ.เวียงแก่น|ต.ม่วงยาย": 1301, "อ.เวียงแก่น|ต.ปอ": 1302,
                        "อ.เวียงแก่น|ต.หล่ายงาว": 1303, "อ.เวียงแก่น|ต.ท่าข้าม": 1304, "อ.ขุนตาล|ต.ต้า": 1401,
                        "อ.ขุนตาล|ต.ป่าตาล": 1402, "อ.ขุนตาล|ต.ยางฮอม": 1403, "อ.แม่ฟ้าหลวง|ต.เทอดไทย": 1501,
                        "อ.แม่ฟ้าหลวง|ต.แม่สลองใน": 1502, "อ.แม่ฟ้าหลวง|ต.แม่สลองนอก": 1503, "อ.แม่ฟ้าหลวง|ต.แม่ฟ้าหลวง": 1504,
                        "อ.แม่ลาว|ต.ดงมะดะ": 1601, "อ.แม่ลาว|ต.จอมหมอกแก้ว": 1602, "อ.แม่ลาว|ต.บัวสลี": 1603,
                        "อ.แม่ลาว|ต.ป่าก่อดำ": 1604, "อ.แม่ลาว|ต.โป่งแพร่": 1605, "อ.เวียงเชียงรุ้ง|ต.ทุ่งก่อ": 1701,
                        "อ.เวียงเชียงรุ้ง|ต.ดงมหาวัน": 1702, "อ.ดอยหลวง|ต.ปงน้อย": 1801, "อ.ดอยหลวง|ต.โชคชัย": 1802,
                ...
                function combine_birth_date(row) {
                    ...
                    // เพิ่มการตรวจสอบค่าที่ได้จากการแปลงเป็นตัวเลข
                    if (isNaN(y) || isNaN(m) || isNaN(d) || y < 1000 || m < 1 || m > 12 || d < 1 || d > 31) {
                        return null;
                    }
                    const gregorianYear = String(y).padStart(4, '0');
                ...
                const incomeMap = {
                    "ไม่มีรายได้": "ไม่มีรายได้", "น้อยมาก": "รายได้น้อยมาก", "น้อย": "รายได้น้อย",
                    "ปานกลาง": "รายได้ปานกลาง", "ปานกลางค่อนสูง": "รายได้ปานกลางค่อนสูง",
                    "สูง": "รายได้สูง", "สูงมาก": "รายได้สูงมาก",
                };

                function normalizeIncomeLabel(label) {
                    if (!label) return label;
                    if (label.includes("รายได้")) return label;
                    return incomeMap[label] ?? label;
                }

                const dataToInsert = results.map(row => [
                    row.gender, row.marital_status, row.monthly_income, row.season, row.age_range,
                    combine_district_from_string(row),
                    combine_subdistrict(row),
                    combine_birth_date(row),
                    1, // province_id: 1 สำหรับเชียงราย
                    row['predict'],
                ]);

                if (dataToInsert.length === 0) {
                    sentResponse = true;
                    return res.json({ status: 'no_data', message: 'ไม่พบข้อมูลที่ถูกต้องสำหรับบันทึก' });
                }

                // ⭐ แก้ไข: ใช้ Bulk Insert เพื่อประสิทธิภาพที่ดีขึ้น
                const sql = `
                    INSERT INTO person (gender, marital_status, monthly_income, season, age_range, district_id, subdistrict_id, birth_date, province_id, predict)
                    VALUES ?
                `;

                con.query(sql, [dataToInsert], (err, dbResult) => {
                    sentResponse = true; // ตั้งค่า flag ก่อนส่ง response
                    if (err) {
                        console.error("❌ MySQL insert error:", err.message);
                        return res.status(500).json({ status: 'error', message: 'Failed to insert data into database.' });
                    }
                    res.json({ status: 'success', inserted: dbResult.affectedRows });
                });
            } catch (err) { // <--- แก้ไข: เพิ่มตัวแปร err ใน catch block
                // ถ้าส่ง response ไปแล้ว ไม่ต้องทำอะไร
                if (sentResponse) return;
                sentResponse = true;
                console.error("❌ JSON parse or data processing error:", err.message);
                console.error("Raw output from Python:", output);
                res.status(500).json({ status: 'error', message: 'Failed to process data. Please check file format.' });
            }
        });

    } catch (err) {
        console.error('An unexpected error occurred:', err);
        return res.status(500).json({ status: 'error', message: 'An unexpected error occurred on the server.' });
    }
});
